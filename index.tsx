...
const App = () => {
    ...
    const [data, setData] = useState(() => {
        ...
        return { 
            ...
            masterTeachers: [], // DANH SÁCH NHÂN SỰ TẬP TRUNG (id: GV001, GV002, ...)
            weeklyRecords: {} 
        };
    });
                    ...
                    <div className="mb-10 bg-white border-4 border-blue-50 p-8 rounded-[2rem] animate-fadeIn shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex justify-end mt-8 gap-3">
                            <button onClick={() => {
                                ...
                                // Nếu giáo viên chưa có trong tuần, thêm vào list teachers tuần
    ...
    const ReportTab = () => {
        ...
        const subjStats = useMemo(() => {
            ...
            // Lọc ra các môn học độc lập (không phải phân môn của môn khác)
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border-4 border-blue-50 shadow-xl space-y-6">
                            <div className="flex flex-col gap-4">
                                <div className="space-y-1">
                                    ...
                                    <input type="text" placeholder="Nhập họ tên..." value={newName} onChange={e => setNewName(e.target.value)} className="w-full p-4 bg-slate-50 rounded-xl font-bold border-none outline-none shadow-inner"/>
                                </div>
                                ...
                                {data.subjectConfigs.map((s: any, i: number) => (
                                    <div key={i} className={`p-5 rounded-[1.5rem] border transition-all ${s.parent ? 'bg-blue-50/20 border-blue-50 ml-6' : 'bg-white border-slate-50 shadow-sm'}`}>
                                        <div className="flex justify-between items-center mb-3">
                                            ...
                                            {!s.parent && !['Toán', 'Ngữ văn', 'Tiếng Anh'].includes(s.name) && (
                                                <button onClick={() => updateData({ subjectConfigs: data.subjectConfigs.filter((x:any)=>x.name !== s.name) })} className="text-slate-200 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><X size={16}/></button>
                                            )}
                                        </div>
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 overflow-x-hidden selection:bg-blue-100 selection:text-blue-800">
            <header className="bg-white border-b border-slate-100 p-3 sticky top-0 z-50 shadow-sm">
                <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="font-black text-xl tracking-tighter text-slate-800 uppercase italic leading-none">GIẢNG DẠY THCS <span className="text-blue-600 text-[10px] align-top font-black italic">PRO v9.0</span></h1>
            ...
            <footer className="p-8 text-center text-[10px] font-black uppercase text-slate-300 tracking-[0.5em] italic flex items-center justify-center gap-3">
                <UserCheck size={16}/> Professional Edition • v9.0
            </footer>
        </div>
    );
};
